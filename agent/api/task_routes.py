...
from models import QueueLoadSummary, TaskEvent, TaskProgressSnapshot
...
def create_router(app_state) -> APIRouter:
    ...
    async def get_recent_events(
        ...
        logger.info(
            "API /events/recent called with session env=%s",
    ...
    async def get_active_tasks(
        session: Session = Depends(get_db), active_env=Depends(get_active_env)
    ):
    ...
    @router.get("/queues/load", response_model=list[QueueLoadSummary])
    async def get_queue_load(
        session: Session = Depends(get_db), active_env=Depends(get_active_env)
    ):
        """Get queue load snapshot grouped by queue."""
        task_service = TaskService(session, active_env=active_env)
        queue_summary = task_service.get_queue_load_summary()

        known_queues: set[str] = set()
        if app_state.monitor_instance:
            try:
                inspect_result = app_state.monitor_instance.app.control.inspect(
                    timeout=1.0
                ).active_queues()
                for queue_entries in (inspect_result or {}).values():
                    for queue_data in queue_entries or []:
                        queue_name = queue_data.get("name")
                        if queue_name:
                            known_queues.add(queue_name)
            except Exception:  # pylint: disable=broad-except
                logger.debug("Unable to inspect active queues", exc_info=True)

        if active_env and active_env.queue_patterns:
            known_queues = {
                queue_name
                for queue_name in known_queues
                if any(
                    fnmatch.fnmatch(queue_name, pattern) for pattern in active_env.queue_patterns
                )
            }

        queue_map = {entry["queue"]: entry for entry in queue_summary}
        sampled_at = (
            queue_summary[0]["sampled_at"]
            if queue_summary
            else datetime.now(timezone.utc).isoformat()
        )

        for queue_name in sorted(known_queues):
    ...
    async def get_recent_failed_tasks(
        ...
        lookback_hours = (
            hours if hours is not None else config_service.get_task_issue_lookback_hours()
        )
    ...
    async def resolve_task(
        ...
        return {
            ...
            "resolved_at": (
                ensure_utc_isoformat(resolution.resolved_at) if resolution.resolved_at else None
            ),
        }
    ...
    async def retry_task(task_id: str, session: Session = Depends(get_db)):
        ...
        orphaned_task = (
            session.query(TaskEventDB).filter_by(task_id=task_id, is_orphan=True).first()
        )
