    ...
    def validate_args(cls, v):
        ...
        if isinstance(v, list):
            ...
            return (
                ...
                else ([] if sanitized is None else [sanitized])
            )
...
class QueueLoadSummary(BaseModel):
...
class WorkerInfo(BaseModel):
    ...
    class Config:
        ...
        json_encoders = {
            datetime: lambda v: (
                v.replace(tzinfo=timezone.utc).isoformat() if v.tzinfo is None else v.isoformat()
            )
        }
    ...
    def from_celery_event(cls, event: dict) -> "WorkerEvent":
        ...
        return cls(
            ...
            timestamp=datetime.fromtimestamp(
                event.get("timestamp", datetime.now(timezone.utc).timestamp()), tz=timezone.utc
            ),
...
class CircuitBreakerConfig(BaseModel):
    ...
    max_executions: int = Field(
        default=1, ge=1, description="Number of executions allowed per window"
    )
